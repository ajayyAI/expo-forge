...
// from the account hard-delete pass so app authorization is removed from
// Apple's side, per Apple's account-deletion requirement:
//
...
// ES256 client-secret JWT (rotated periodically, since Apple caps its lifetime
// at 180 days). Best-effort: a failed revoke logs and returns rather than
// throwing, because the user has already confirmed account deletion and the
// local rows are about to be wiped anyway.
export const revokeRefreshToken = internalAction({
  ...
  handler: async (_ctx, { refreshToken }) => {
    ...
    if (!(clientId && clientSecret)) {
      console.warn(
        "[apple] revoke skipped: APPLE_CLIENT_ID or APPLE_CLIENT_SECRET unset"
      );
      return null;
    }
    ...
    try {
      ...
      console.warn(
        `[apple] revoke threw: ${err instanceof Error ? err.message : String(err)}`
      );
    }
    return null;
  },
