...
// Paginated walk of the pushTokens table that purges revoked rows past their
// 30-day retention and stale active rows past 90 days. Called with no args so
// the walk starts from a fresh cursor each tick; it self-reschedules through
// the page cursor until the table is fully scanned.
crons.daily(
...
// Hourly TTL sweep of expired App Attest challenges. Bounded front-delete via
// `internal.appAttestStore.cleanupChallenges` (self-reschedules while a full
// batch still has expired rows).
crons.hourly(
