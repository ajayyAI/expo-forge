  ...
  triggers: {
    user: {
      onCreate: async (ctx, authUser) => {
        // Identity fields live on the Better Auth user record, not here.
...
// Google OAuth is registered only when its credentials are present on the
// deployment. Better Auth logs a warning on every request for an empty
// provider, and the client hides the button via the capability checks when
// the env vars are unset, so registering an empty provider serves no purpose.
const googleConfigured =
  env.google.clientIds.length > 0 && !!env.google.clientSecret;

// Apple is registered only when its credentials are present, for the same
// reason.
...
export const createAuth = (ctx: GenericCtx<DataModel>) =>
  betterAuth({
    ...
    trustedOrigins: [
      ...
      // `exp+<scheme>://`. Wildcards match the full origin (e.g. `exp://*`
      // matches `exp://192.168.1.1:8081`). Production builds drop these.
    ...
    emailAndPassword: {
      ...
      // Convex env var. The default (unset) leaves it `false` so sign-up
      // creates verified accounts immediately and the user can sign in
      // without an OTP. No Resend configuration needed to get running on
      // the iOS Simulator. Flip it to `true` once Resend is provisioned.
    ...
    socialProviders: {
      // Google id-token verification accepts tokens issued for any of the
      // configured client ids (web / iOS / Android), passed as an array of
      // accepted audiences so the same provider works on every platform.
      ...(googleConfigured
        ? {
            google: {
              clientId: env.google.clientIds,
    ...
    // This app uses the email-OTP flow, so password reset and verification
    // hit /email-otp/* rather than the link-based endpoints.
    rateLimit: {
      enabled: true,
      window: ONE_MINUTE,
      max: 100,
    ...
    plugins: [
      ...
      emailOTP({
        ...
        // actually required. The minimal setup short-circuits the OTP
...
export async function safeGetAuthenticatedUser(
  ...
  const avatarUrl = hasUploadedAvatar
    ? await ctx.storage.getUrl(user.avatar as NonNullable<typeof user.avatar>)
    : (authUser.image ?? null);
  ...
  return {
    ...
    displayUsername:
      (authUser as { displayUsername?: string | null }).displayUsername ?? null,
    avatarUrl,
    hasUploadedAvatar,
  };
}
 ...
 * client hide buttons that would fail at submit (e.g. Apple or Google Sign In
 * with empty credentials, OTP sign-in or password reset with
 * `REQUIRE_EMAIL_VERIFICATION` unset). Returns booleans only, never leaks the
 * credentials.
 *
 * `emailFeatures` is true when `REQUIRE_EMAIL_VERIFICATION` is set on the
 * Convex deployment env. When false, the client hides OTP sign-in, password
 * reset, and change-email; the only working flow is email + password
 * sign-up/sign-in. That is the minimal path: users get into the app without
 * configuring Resend or any DNS.
 */
export const getEnabledProviders = query({
  ...
  returns: v.object({
    apple: v.boolean(),
    google: v.boolean(),
    emailFeatures: v.boolean(),
  }),
  handler: () => {
    const apple =
      !!process.env.APPLE_CLIENT_ID && !!process.env.APPLE_CLIENT_SECRET;
    const google = env.google.clientIds.length > 0 && !!env.google.clientSecret;
    const emailFeatures = env.requireEmailVerification;
    return { apple, google, emailFeatures };
  },
});
