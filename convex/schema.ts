...
export default defineSchema(
  {
    ...
    // user component and are merged in at read time.
    //
    // `deletedAt` is the soft-delete tombstone. Set when the user requests
    // account deletion. A daily cron permanently removes the row 30 days
    // after this timestamp. Within the window, the user can sign back in
    ...
    users: defineTable({
      ...
      // Cron scan ordered by tombstone time. Rows with `deletedAt` unset sort
      // first in this index, so the hard-delete pass MUST range-filter to skip
      // them, e.g. `.withIndex("by_deletedAt", q => q.gt("deletedAt", 0))`.
    ...
    //     resurrect a dead token; a daily cleanup cron hard-deletes
    //     revoked rows after 30 days.
    pushTokens: defineTable({
      userId: v.id("users"),
      token: v.string(),
      deviceType: v.union(v.literal("ios"), v.literal("android")),
      createdAt: v.number(),
      updatedAt: v.number(),
      ...
      // Lets a sender or audit narrow to (revoked, updatedAt) ranges. The
      // daily cleanup cron does NOT use this index: it cursor-paginates the
      // whole table once per tick so the walk provably terminates instead of
      // re-reading the front of an index forever (see pushTokens.cleanupStale).
    ...
    // Verified App Attest keys. Created on first attestation verification
    // and referenced by every subsequent assertion verification.
    // `publicKey` is the DER-encoded SubjectPublicKeyInfo, `counter` is
    // the monotonic signCount from authenticatorData.
    appAttestKeys: defineTable({
      ...
      // each successful assertion verification. Reject any assertion with
      // a counter not strictly greater than this.
      counter: v.number(),
